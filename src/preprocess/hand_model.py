...
class HandModel:
    def __init__(
        ...
        if not os.path.exists(contact_indices_path):
            print("do not load contact, handmodel only for visualization")
            self.hand_pose = None
        ...
        self.pose_distrib = torch.load(pose_distrib_path, map_location=self.palm_contact_indices.device)
        if self.left_hand:
    ...
    def mirror_pose(self, hand_pose):
        """Mirror right hand parameters to left hand parameters"""
        hand_pose_mirrored = hand_pose.detach().clone()
        
        # Translation X
        # hand_pose_mirrored[:, 0] *= -1
        
        # Rotation Y, Z
        hand_pose_mirrored[:, 4] *= -1  
        hand_pose_mirrored[:, 5] *= -1
        
        # Thetas Y, Z components (starting from index 6)
        thetas_start = 6
        hand_pose_mirrored[:, thetas_start + 1::3] *= -1  # Y components
        hand_pose_mirrored[:, thetas_start + 2::3] *= -1  # Z components
        
        hand_pose_mirrored.requires_grad_()
        return hand_pose_mirrored
    
    def set_parameters(self, hand_pose, contact_point_indices=None, distance_point_indices=None, skip_left_mirror=False):
        """
        ...
        """
        if self.left_hand and not skip_left_mirror: # skip left mirror: parameter is already mirrored
            hand_pose = self.mirror_pose(hand_pose)
        ...
        if self.contact_indices is None:
            return # no contact points, this is just for visualization
        ...
        self.all_contact_points = self.vertices[torch.arange(len(hand_pose)).unsqueeze(1), self.contact_indices]

        if distance_point_indices is None:
            distance_point_indices = contact_point_indices

        self.distance_points = self.vertices[torch.arange(len(hand_pose)).unsqueeze(1), self.contact_indices[distance_point_indices]]
